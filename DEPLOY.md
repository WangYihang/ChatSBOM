...
**`sbom lock` does not run in the collector, on purpose.** It starts a
...
whatever those resolvers execute — a Gemfile is Ruby, a POM runs build
plugins. Run it deliberately, on the host:

```bash
uv run chatsbom sbom lock --language java --limit 100
```
